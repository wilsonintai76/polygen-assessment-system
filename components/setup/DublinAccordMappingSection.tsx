...
interface DublinAccordMappingSectionProps {
  daClusters: Record<string, string>;
  onUpdate: (da: Record<string, string>) => void;
}

export const DublinAccordMappingSection: React.FC<DublinAccordMappingSectionProps> = ({ daClusters, onUpdate }) => {
  const addDa = () => {
    // Generate a temporary unique key so the user can manually define the Dublin Accord code
    const tempKey = `NEW_DA_${Date.now()}`;
    onUpdate({ ...daClusters, [tempKey]: '' });
  };

  const removeDa = (key: string) => {
    const newDa = { ...daClusters };
    delete newDa[key];
    onUpdate(newDa);
  };

  const updateDaDesc = (key: string, value: string) => {
    onUpdate({ ...daClusters, [key]: value });
  };

  const updateDaKey = (oldKey: string, newKey: string) => {
    if (oldKey === newKey || !newKey.trim()) return;
    const newDa: Record<string, string> = {};
    (Object.entries(daClusters) as [string, string][]).forEach(([k, v]) => {
      if (k === oldKey) newDa[newKey] = v;
      else newDa[k] = v;
    });
    onUpdate(newDa);
  };
          ...
          <div key={key} className="bg-white p-5 rounded-2xl border border-[#eef6ff] shadow-sm hover:shadow-md transition group relative">
            ...
            <div className="flex items-center mb-3">
              <input 
                ...
                placeholder="e.g. DK1"
                onChange={(e) => updateDaKey(key, e.target.value)}
              />
            </div>
            <textarea 
              ...
              value={desc} 
              onChange={e => updateDaDesc(key, e.target.value)} 
            />
          </div>
        ))}
        {Object.keys(daClusters).length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-[#cbd5e1] italic text-sm py-20 text-center">
            <span className="text-4xl mb-4 block">🧬</span>
            No Dublin Accord attributes defined. Click + ADD to start.
          </div>
        )}
      </div>
    </div>
